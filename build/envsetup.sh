...
- githubremote:    Add git remote for PixelExtended Github.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/PixelExtended-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/PixelExtended/$PROJECT
